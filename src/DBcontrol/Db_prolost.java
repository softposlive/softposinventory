...
public class Db_prolost {
    ...
    public Db_prolost() throws SQLException{
...
//            String sql = "select max(substring(R_No,4,6)) as xxx from htranout";
    ...
    public int maxRque(String rno){
        int maxrque=0;
        try {
            String sql = "select max(r_que)+1 as max from prolost "
                    + "where r_no = '"+rno+"'";
            ResultSet rs = stmt.executeQuery(sql);
            if(rs.next()){
    ...
    public boolean insert(Prolost row){
        String sql = "INSERT INTO prolost " +
                "VALUES ("+
                "'"+row.getR_no()+"'"+ ","+
                row.getR_que()+ ","+
                "'"+row.getR_pcode()+"'"+ ","+
    ...
    public boolean update(Prolost row){
    ...
    public boolean delete(String r_no,int que){
        String sql = "DELETE FROM prolost" +
    ...
    public boolean deleteProlost(String r_no,String pcode){
        String sql = "DELETE FROM prolost" +
    ...
    public boolean seek(String r_no){
        String sql = "SELECT r_no FROM prolost WHERE r_no='"+r_no+"'";
        try {
            ResultSet rs = stmt.executeQuery(sql);
            if (rs != null) {
                if (rs.next()) {
    ...
    private Prolost getRecordAt(String r_no , int que){
        ResultSet rs;
        String sql;
        Prolost hrb = new Prolost();
        try {
            sql = "SELECT * FROM prolost WHERE r_no='"+r_no+"' AND r_que="+que;
